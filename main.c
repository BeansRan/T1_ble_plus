...
#define BLE_HRM_APP_SUPPORT

#define IS_SRVC_CHANGED_CHARACT_PRESENT  1                                          /**< Include or not the service_changed characteristic. if not enabled, the server's database cannot be changed for the lifetime of the device*/

#define DEVICE_NAME                      "Nordic_HRM"                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "ORVIBO T1"                                /**< Manufacturer. Will be passed to Device Information Service. */
...
static void gap_params_init(void)
{
    ...
    err_code = pstorage_load((uint8_t *)&dev_name, &dest_block_id, sizeof(dev_name_t), 0); 
    if(err_code == NRF_SUCCESS)
    {
        if(true == dev_name.updt)
        {
            err_code = sd_ble_gap_device_name_set(&sec_mode,
                                                  (const uint8_t *)dev_name.data,
                                                  dev_name.length);
            APP_ERROR_CHECK(err_code);
        } 
        else
    ...
    {
        app_trace_log("pstorage load device name failed.\r\n");
//        APP_ERROR_CHECK(err_code);
    }
...
{
    ble_adv_name_manager_on_ble_evt(p_ble_evt);
...
static void advertising_init(void)
{
    ...
    advdata.uuids_complete.uuid_cnt = sizeof(m_adv_uuids) / sizeof(m_adv_uuids[0]); /* < Calculate uuids' number */ 
